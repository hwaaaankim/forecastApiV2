...
//    	}
	...
	public Member signup(MemberLoginRequestDTO memberLoginRequestDTO) {
		if (memberRepository.findOneByUsername(memberLoginRequestDTO.getUsername()).orElse(null) != null) {
			throw new RuntimeException("이미 가입되어 있는 유저입니다.");
		}

		// 유저 정보를 만들어서 save
		Member member = Member.builder().username(memberLoginRequestDTO.getUsername())
				.password(passwordEncoder.encode(memberLoginRequestDTO.getPassword()))
				.nickname(memberLoginRequestDTO.getNickname()).roles(memberLoginRequestDTO.getRoles())
//                .administrationDate(new Date())
	...
	public Optional<Member> getMyUserWithAuthorities() {
		...
		return SecurityUtil.getCurrentUsername().flatMap(memberRepository::findOneByUsername);
	}
